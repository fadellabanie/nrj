    ...
    <div class="card card-flush mt-6 mt-xl-9">
        <div class="card-header mt-5">
            <div class="card-title flex-column">
                <h3 class="fw-bolder mb-1">{{__("Shows")}}</h3>
                    ...
                    <table id="kt_profile_overview_table"
                        ...
                        <thead class="fs-7 text-gray-400 text-uppercase">
                            <tr role="row">
                                ...
                                <th wire:click="sortBy('from')" data-sort="{{$sortDirection}}" class="min-w-90px">
                                    {{__("From")}}
                                    <x-sort field="from" sortBy="{{$sortBy}}" sortDirection="{{$sortDirection}}">
                                    </x-sort>
                                </th> 
                                <th wire:click="sortBy('to')" data-sort="{{$sortDirection}}" class="min-w-90px">
                                    {{__("To")}}
                                    <x-sort field="to" sortBy="{{$sortBy}}" sortDirection="{{$sortDirection}}">
                                    </x-sort>
                                </th>
                            ...
                            <tr wire:loading.class="opacity-50">
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="me-5 position-relative">
                                            <div class="symbol symbol-35px symbol-circle">
                                                <img alt="Pic" src="{{asset($show->image)}}">
                                            </div>
                                        </div>
                                        <div class="d-flex flex-column justify-content-center">
                                            <a href="{{route('admin.shows.show',$show)}}"
                                                class="fs-6 text-gray-800 text-hover-primary">{{$show->title}}</a>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-end flex-shrink-0">
                                        <x-edit-button href="{{route('admin.shows.edit',$show)}}" />
                                        <x-delete-record-button wire:click="confirm({{ $show->id }})" />
                                    </div>
                                </td>
                            </tr>
                            @empty
